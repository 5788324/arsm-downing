...
async def test():
    ...
    rj="RJ99962"; src=f"/tmp/{rj}"; tgt=f"/tmp/{rj}_m"; os.makedirs(f"{src}/sub",exist_ok=True)
    ...
    db.upsert_download(f"{rj}:t2",rj,"t2",f"{src}/sub/t2.mp3","registered",200,200); db.commit()
    engine=MigrationEngine(db); res=engine.migrate_one(rj,src,tgt)
    assert res["success"]
    for r in db.conn.execute("SELECT local_path FROM downloads WHERE rj_id=?",(rj,)):
        assert r["local_path"].startswith(tgt),f"should start with {tgt}: {r['local_path']}"
