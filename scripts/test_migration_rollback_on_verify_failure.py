...
async def test():
    print(f"\n{'='*60}\n  migration rollback on verify failure\n{'='*60}\n")
    ...
    rj="RJ99808"; src=f"/tmp/{rj}"; tgt=f"/tmp/{rj}_vfy"
    ...
    # Monkey-patch _verify_dir to always fail
    orig=MigrationEngine._verify_dir
    MigrationEngine._verify_dir=lambda self,src,dst: False
    engine=MigrationEngine(db); res=engine.migrate_one(rj,src,tgt)
    MigrationEngine._verify_dir=orig
    assert not res["success"],"should fail"
    assert os.path.exists(src),"source should remain"
    print(f"  ✓ verify failed → rollback, source preserved")
    shutil.rmtree(tgt+".tmp_migrating",ignore_errors=True)
