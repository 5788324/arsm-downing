...
async def test():
    ...
    rj="RJ99967"; d=f"/tmp/{rj}"; os.makedirs(d,exist_ok=True)
    ...
    db.upsert_download(f"{rj}:t1",rj,"t1",f"{d}/t1.mp3","queued",0,100); db.commit()
    safe=db.get_safe_migratable_works()
    assert not any(w["rj_id"]==rj for w in safe),f"queued should be rejected"
    print(f"  ✓ queued download → rejected from safe list")
