...
- 历史测试数量很多且质量不一；现已建立 158 项可信 portable pytest 门，但 200 个旧脚本尚未全部迁移。
...
处理状态（2026-07-20）：已完成。`move_work_to_path()` 委托统一路径事务，同步 `works`、`downloads`、`library_items`、`library_index`；迁移 post-verify 现在显式核验 `library_items` 和 `library_index` 的目标路径。

### P0-05 迁移可能漏检嵌套 `.part`

`_has_part_files()` 旧实现只扫描源目录顶层。T8A 已改为递归扫描，并同时拒绝源根目录或内部 symlink。
...
### P1-10 迁移完成判定偏弱（T8A 已关闭）

- copy verify 只比较总文件数和总字节数，不逐相对路径比较。
- 删除源目录使用 `ignore_errors=True`，即使删除失败也可能返回 success。
- 旧迁移空间估算使用 DB `size_bytes`。T8A 已改为磁盘 manifest 实测，并按相对路径、逐文件大小和哈希校验。
...
| 迁移 | T8A 代码级收口：递归 part/symlink、磁盘实测、四表事务、逐文件验证和删除确认；待 Windows 证据 |
...
| 测试/CI | 174 项 portable 门、本地 HTTP 集成及 T6/T8A 沙盒验收已建立；历史脚本迁移与远端 CI 首跑仍待完成 |
