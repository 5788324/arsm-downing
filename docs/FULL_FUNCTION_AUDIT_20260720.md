...
> 修复状态：TAKEOVER-T0~T4 与 T5A 已完成 external intake 安全收口、统一 pytest 门、活跃 DB 在线快照，以及下载 200/206/416、断点进度和主要 UI 控制语义修复；真实 external intake 与 Windows live/UI 验收仍冻结或待执行。
...
- 历史测试数量很多且质量不一；现已建立 125 项可信 portable pytest 门，但 200 个旧脚本尚未全部迁移。
...
处理状态（2026-07-20）：下载准备、恢复、重连、批量控制和关闭回调已统一经过后台 loop/UI queue；LibraryView/ToolsView 其他长操作仍需继续审查。
