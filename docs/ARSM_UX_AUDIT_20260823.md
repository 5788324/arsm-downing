...
## 2026-08-30 浏览器最终复核补充

2026-08-23 的 `NOT RUN` 是当日快照，以下最终实机证据补齐并取代相应阻塞项：

1. Edge 详情页显示状态和下载入口正常；隔离测试 RJ 入队后回写“已排队 / 查看下载”；
2. 重复提交返回 HTTP 409 `already_queued`，多标签没有创建第二个作品任务；
3. ARSM 完全退出后页面无需刷新自动显示未连接；同一隔离 Profile 重启后无需刷新自动恢复；
4. Edge 100% / 125% / 150% 缩放正常；Chrome 深色/浅色外观正常；
5. Chrome 在 `asmr.one` 和 `www.asmr.one` 详情页均显示“已暂停 / 查看下载”；
6. Chrome 移除 ARSM 扩展后，网页控件按预期消失；隔离 ARSM 桥接继续返回 HTTP 200，数据库保持 1 个作品 / 60 条下载；
7. 隔离下载代理故意设置为不可用地址，实际媒体写入为 0 字节；该安全配置不作为产品下载失败；
8. 最终自动门禁为 `411 passed, 3 skipped`；真实 `E:\arsm`、正式数据库和正式队列零接触。
